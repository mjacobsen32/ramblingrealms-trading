...
def setup():
    ...
    """
    user = User.load()
    etrade = Prompt("Do you want to set up E-Trade? (y/n)", default="n")
    if etrade.lower() == "y":
        # E-Trade setup
        user.etrade_api_key_path = Prompt("Enter your E-Trade API key path")
        user.etrade_api_secret_path = Prompt("Enter your E-Trade API secret path")
        
        etrade_authenticate = Prompt("Do you want to authenticate E-Trade now? (y/n)", default="n")
        if etrade_authenticate.lower() == "y":
            # E-Trade authentication
            from trading.cli.etrade.etrade import authenticate
            authenticate()
    polygon = Prompt("Do you want to set up Polygon? (y/n)", default="n")
    if polygon.lower() == "y":
        # Polygon setup
        user.polygon_access_token_path = Prompt("Enter your Polygon access token path")

...
def benchmark_environment(
    env_class, env_name, num_episodes=5, num_steps_per_episode=100
):
    ...
    for episode in range(num_episodes):
        ...
        print(
            f"Episode {episode + 1}: {episode_steps} steps in {episode_time:.3f}s "
    ...
    print(
        f"  Achievement: {(avg_steps_per_sec / TARGET_STEPS_PER_SEC) * 100:.1f}% of target"
    )
